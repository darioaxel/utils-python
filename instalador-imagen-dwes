...
def check_docker_installed():
    ...
    check_cmd = "dpkg -l | grep docker-ce"
    return os.system(check_cmd) == 0

# Si Docker CE existe, desinstalar y borrar datos residuales
if check_docker_installed():
    print("Docker CE encontrado. Desinstalando y borrando datos residuales...")
    ...
    run_command('sudo rm -rf /var/lib/containerd')
else:
    print("Docker CE no está instalado. Procediendo con la instalación...")
...
# --- NUEVAS INSTALACIONES SOLICITADAS ---

# Instalar IntelliJ IDEA Community Edition descargando y descomprimiendo
intellij_tarball = "ideaIC.tar.gz"
run_command(f'wget -O {intellij_tarball} "https://download.jetbrains.com/idea/ideaIC-latest.tar.gz"')
run_command('sudo tar -xzf ideaIC.tar.gz -C /opt/')
# Renombrar la carpeta descomprimida para facilitar acceso (puede variar versión)
run_command('sudo mv /opt/idea-IC-* /opt/intellij-idea-community')
# Crear enlace simbólico para comando global
run_command('sudo ln -sf /opt/intellij-idea-community/bin/idea.sh /usr/local/bin/intellij')

# Instalar la versión estable actual de Python y pip
...
# Instalar PHP 8.4 y extensiones básicas
run_command('sudo apt update')
run_command('sudo add-apt-repository -y ppa:ondrej/php')
run_command('sudo apt update')
run_command('sudo apt install -y php8.4')

# Instalar Apache última versión estable
run_command('sudo apt update')
